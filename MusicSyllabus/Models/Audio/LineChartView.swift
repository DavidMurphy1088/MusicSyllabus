...
struct LineChartView: View {
    let dataPoints: [Double] // Array of data points
            ...
            if dataPoints.count > 0 {
                GeometryReader { geometry in
                    Path { path in
                        ...
                        let yScale = geometry.size.height / (dataPoints.max()! / 0.5)
                        
                        // Start drawing the path at the first data point
                        path.move(to: CGPoint(x: 0, y: geometry.size.height - dataPoints[0] * yScale))
                        
                        // Draw lines to connect the data points
                        for index in 1..<dataPoints.count {
                            let point = CGPoint(x: CGFloat(index) * xScale, y: geometry.size.height / 2.0 - dataPoints[index] * yScale)
